...
public class InsFuelEventController  {
    ...
    private static final String SQL_SELECT_VEHICLES = "SELECT * FROM VEHICLE_TABLE WHERE IS_ACTIVE=TRUE ORDER BY VIN";
    private static final String SQL_SELECT_FUEL_EVENTS = "SELECT * FROM FUEL_EVENT WHERE EVENT_TIME IN (SELECT EVENT_TIME FROM VEHICLE_FUEL_TABLE WHERE VIN = ?) ORDER BY EVENT_TIME";
    ...
    private static final String SQL_UPDATE_FUEL_EVENT = "UPDATE FUEL_EVENT SET ODOMETER = ?, TOTAL_PRICE = ?, NUM_GAL = ?, IS_FULL_TANK = ? WHERE EVENT_TIME = ?";
    private static final String SQL_DELETE_FUEL_EVENT = "DELETE FROM FUEL_EVENT WHERE EVENT_TIME = ?";
    ...
    public void initialize() {
        ...
        vehicles.clear();
        vehicles.addAll(dbQueries.DBSelect(SQL_SELECT_VEHICLES, data.toArray(), currVehicle));
    ...
    private void DisplayFuelEvent(FuelEvent FE){
        ...
        totalPriceTextID.setText(((Double)FE.getTotalPrice()).toString());
        numGallonsTextID.setText(((Double)FE.getNumGallons()).toString());
        filledTankCheckID.setSelected(FE.isFilledTank());

        currFuelEvent.setFuelEvent(FE.getEventTime(), FE.getOdometer(), FE.getTotalPrice(), FE.getNumGallons(), FE.isFilledTank());
   }
    ...
    private void CalcFuelEco(){
        ...
        ArrayList<Double> gallonsBetweenFullTanks = new ArrayList<>();
        ...
        for(int i = 0; i < milesBetweenFullTanks.size(); ++i){
            ...
            Double result = (double)milesBetweenFullTanks.get(i) / gallonsBetweenFullTanks.get(i);
            lastFillEcoID.setText(result.toString());
            result = (double)miles / gallons;
            overallEcoID.setText(result.toString());
        }
    }
...
//      entry into FUEL_EVENT table
        FuelEvent FE = new FuelEvent(
                ...
                Double.parseDouble(totalPriceTextID.getText()),
                Double.parseDouble(numGallonsTextID.getText()), 
...
//              entry into VEHICLE_FUEL_TABLE                
    ...
    void LoadButtonPressed(ActionEvent event) {
            ...
            if (result == 1) {
                result = dbQueries.DBUpdate(SQL_DELETE_FUEL_EVENT, data.toArray());
                if (result == 1) {
